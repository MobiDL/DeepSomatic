...
workflow DeepSomatic {
	...
	call deepvariant.deepvariant {
		input:
			threads = 12,
			path_exe="run_deepsomatic",
			refFasta = fasta,
			bam = bam,
			outputPath = "~{outputPath}/",
			subdir = "1-deepsomatic/",
			bed = bed,
			tumor=true,
			suffix=".ds",
			model="WGS_TUMOR_ONLY",
			default_pon_filtering = true,
			make_examples_extra_args= ["vsc_min_fraction_snps=0.03","vsc_min_fraction_indels=0.05"],
			apptainer_img = "deepsomatic:1.9.0"
	}
	...
	call bcftools.sort {
		input:
			threads = 12,
			vcf = select_first([view.outputvcf, deepvariant.outputVcf]),
			outputPath = "~{outputPath}/",
			subdir = "3-sort/"
	}
